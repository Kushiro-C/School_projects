...
/**
 * move a directory "source" (already in the tar)
 * in the tar at the destination "dest"
 */
int move_directory(int fd, char* source, char* dest) {
    ...
    // else we change the name of the source header
    int inDirectory = 1;
    ...
    if (lseek(fd, 0L, SEEK_SET) < 0) {
        close(fd);
        perror("move_directory seek1");
        exit(EXIT_FAILURE);
    }

    // update links
    if (inDirectory == 0)
        change_link(fd, source, dest);
    else
        change_link(fd, source, s);

    int cpt = 0;
    char content_name[100];
    char tmp[100];

    // move files by changing their names
    while ((re = read(fd, &head2, BLOCKSIZE)) > 0) {
        if (strncmp(source, head2.name, strlen(source)) == 0) {
            ...
            if (strcmp(head_source.name, head2.name) == 0) {
                if (inDirectory == 0) {
                    memset(head2.name, '\0', 100);
                    strcpy(head2.name, add_slash_at_the_end(dest));
                }
                else {
                    memset(head2.name, '\0', 100);
                    strcpy(head2.name, s);
            ...
            if (lseek(fd, -BLOCKSIZE, SEEK_CUR) < 0) {
                close(fd);
                perror("move_directory seek error");
                exit(EXIT_FAILURE);
            }
            if (write(fd, &head2, BLOCKSIZE) < 0) {
                close(fd);
                perror("move_directory write error");
                exit(EXIT_FAILURE);
            }
            cpt ++;
        }
    ...
    if (re < 0) {
        close(fd);
        perror("move_directory read error");
        exit(EXIT_FAILURE);
    }
    if (lseek(fd, current, SEEK_SET) < 0) {
        close(fd);
        perror("move_directory seek2");
        exit(EXIT_FAILURE);
    }
    if (cpt == 0) {
...
/**
 * move no directory (already in the tar)
 * source header to dest header
 */
int move_noDirectory(int fd, char* source, char* dest) {
    if (strcmp(dest, source) == 0)
        return 1;
    ...
    while ((re = read(fd, &head2, BLOCKSIZE)) > 0) {
        ...
        if (strcmp(head2.name, source) == 0) {
            ...
            if (lseek(fd, -BLOCKSIZE, SEEK_CUR) < 0) {
                close(fd);
                perror("move_noDirectory seek error");
                exit(EXIT_FAILURE);
            }

            if (write(fd, &head2, BLOCKSIZE) < 0) {
                close(fd);
                perror("move_noDirectory write error");
                exit(EXIT_FAILURE);
            }
            if (lseek(fd, current, SEEK_SET) < 0) {
                close(fd);
    ...
    if (re < 0) {
        close(fd);
        perror("move_noDirectory read error");
        exit(EXIT_FAILURE);
    }
    if (lseek(fd, current, SEEK_SET) < 0) {
        close(fd);
...
/**
 * move files into the tar to the tar
 */
int move_tarToTar(int argc, char ** argv) {
    char* tar = get_path_until_tar(argv[1]);
    char* source = get_path_after_tar(argv[1]);
    ...
    if (argHead.typeflag != '5')
        return move_noDirectory(fd, source, dest);
    return move_directory(fd, source, dest);

    return 0;
}

/**
 * move no directory (already in the tar) out the tar
 */
int move_no_directory_out(int fd, char* source, char* dest) {
    struct header head2;
    int re;
    int size;
    ...
    if (file_exist(dest) == 0) {
            ...
            if (strcmp(head2.name, source) == 0) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if ((re2 = read(fd, &contents, BLOCKSIZE)) < 0) {
                        close(fd2);
                        close(fd);
                        perror("move_no_directory_out read in error");
                        exit(EXIT_FAILURE);
                    }
        ...
        if (re < 0) {
            close(fd);
            perror("move_no_directory_out read error");
            exit(EXIT_FAILURE);
        }
        ...
        if (lstat(dest, &stat_dest) < 0) {
            close(fd);
            perror("move_no_directory_out lstat error");
            exit(EXIT_FAILURE);
        }

        if (! S_ISDIR(stat_dest.st_mode)) {
            remove(dest);
            return move_no_directory_out(fd, source, dest);
        } else {
            char tmp[BLOCKSIZE];
            strcpy(tmp, dest);
            strcat(tmp, "/");
            strcat(tmp, get_last_of_path(source, '/'));
            return move_no_directory_out(fd, source, tmp);
        }
    }
...
int move_directory_out(int fd, char* source, char* dest, int allow) {
    ...
    if (allow == 1 || file_exist(dest) == 0) {
        ...
        while ((re = read(fd, &head2, BLOCKSIZE)) > 0) {
            ...
            if (strncmp(source, head2.name, strlen(source)) == 0) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if ((re2 = read(fd, &contents, BLOCKSIZE)) < 0) {
                        close(fd);
                        perror("move_directory_out read in error");
                        exit(EXIT_FAILURE);
                    }
        ...
        if (re < 0) {
            close(fd);
            perror("move_directory_out read error");
            exit(EXIT_FAILURE);
        }
        ...
        if (lstat(dest, &stat_dest) < 0) {
            close(fd);
            perror("move_directory_out lstat error");
            exit(EXIT_FAILURE);
        }
        ...
        if (! S_ISDIR(stat_dest.st_mode)) {
            ...
            if (file_exist(tmp) == 1) {
                ...
                // if it is empty
                rmdir(dest);
                return move_directory_out(fd, source, dest, 1);
            }
        }
    }
...
/**
 * move folders and files in tar to out
 */
int tarToOut(int argc, char ** argv) {
    ...
    if (argHead.typeflag != '5') {
        if (argc == 3)
            ...
            return move_no_directory_out(fd, source, argHead.name); // move to .
    }
    else {
        if (argc == 3)
            return move_directory_out(fd, source, dest, 0);
        else
            return move_directory_out(fd, source, argHead.name, 0);
    }
...
/** /!\ not used yet
 * remove a out directory
 */
void remove_out_directory(char* directory) {
    struct dirent *files;
    char content_name[100];
    char tmp[100];
    DIR *dir = opendir(directory);

    // read all files into the directory
    if (dir != NULL) {
        while ((files = readdir(dir)) != NULL) {
            ...
            if (lstat(file_name, &stat_directory) < 0) {
                perror("remove_out_directory lstat error");
                exit(EXIT_FAILURE);
            }
            if (S_ISDIR(stat_directory.st_mode) == 1)
                remove_out_directory(file_name);
            else
                remove(file_name); // unlink(file_name);
        }
    } else {
        closedir(dir);
        perror("remove_out_directory");
        exit(EXIT_FAILURE);
    }
...
/**
 * move a directory (already in the tar) in the tar
 */
...
/**
 * mv out folders and out files into tar
*/
...
int main(int argc, char ** argv)
{
    ...
    if (argc == 3 && has_tar_in_path(argv[1]) && has_tar_in_path(argv[2])) {
        return move_tarToTar(argc, argv);
    }
